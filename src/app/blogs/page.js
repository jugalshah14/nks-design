...
export default function LifeAtAlcove() {
    ...
    const fetchBlogs = async () => {
      try {
        ...
        const featuredResponse = await fetch(
          "https://admin.newkolkata.in/api/blog-posts?populate[0]=*&populate[1]=Postimage&populate[2]=thumbnailimage&populate[3]=Date&populate[4]=author.image&populate[5]=categories&filters[categories][slug][$eq]=featured&pagination[limit]=-1"
        );
        ...
        const residentialResponse = await fetch(
          "https://admin.newkolkata.in/api/blog-posts?populate[0]=*&populate[1]=Postimage&populate[2]=thumbnailimage&populate[3]=Date&populate[4]=author.image&populate[5]=categories&filters[categories][slug][$eq]=residential&pagination[limit]=-1"
        );
  ...
  // Blog Card Component
  const BlogCard = ({ blog, variant = "side", delay = 0, showDescription = true }) => {
    if (variant === "side") {
      return (
        <SlideUp delay={delay}>
          <a href={`/blogs-detail/${blog.attributes.slug}`} className="block">
            <div className="flex flex-col mx-7 md:mx-0 md:flex-row gap-4 items-stretch font-satoshi hover:opacity-90 transition-opacity">
              <div className="rounded-lg md:mt-7 relative md:min-w-[272px] md:w-[272px] h-[201px] overflow-hidden">
                <Image
    ...
    <main>
      ...
      >
        ...
        <SlideUp delay={0.8}>
          <div className="flex flex-col mx-7 md:mx-0">
              <div className="flex justify-center rounded-lg md:mt-7 relative w-full h-[351px] overflow-hidden">
              <Image
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-7xl mx-auto px-7 md:px-0">
          {residentialBlogs.slice(0, 3).map((blog, index) => (
            <BlogCard
              key={blog.id}
              blog={blog}
              variant="grid"
